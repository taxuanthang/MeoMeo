...
from Load_image import count_class
...
def svm_loss_vectorized(W, X, y, reg):
  list = []
  _, list = count_class("./datasets/recognition/test")
  ...
  num_train = X.shape[0]
  S = X.dot(W)
  correct_class_score = np.choose(y_numeric, S.T).T.reshape(num_train,1)

  margins = np.maximum(0, S - correct_class_score + 1)
  margins[np.arange(margins.shape[0]), y_numeric] = 0
  ...
  F[np.arange(F.shape[0]),y_numeric] = np.sum(-F, axis = 1)
